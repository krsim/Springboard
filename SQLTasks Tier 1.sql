...
Order by descending cost, and do not use any subqueries. */
	...
	AND 
	((m.memid = 0 AND guestcost*slots>30)
 	OR (m.memid != 0 AND membercost*slots>30))
	ORDER BY cost DESC

/* Q9: This time, produce the same result as in Q8, but using a subquery. */

SELECT name, CONCAT_WS(' ', firstname, surname) AS 	fullname,  cost
FROM 
(SELECT bookid, CASE WHEN m.memid = 0 THEN guestcost * slots
	ELSE membercost * slots END AS cost
	FROM
	Members AS m
	INNER JOIN
...
SELECT  a.surname || ', ' || a.firstname AS MemberName,
        b.surname || ', ' || b.firstname AS RecommendedBy
...
/* Q13: Find the facilities usage by month, but not guests */
	...
	SELECT b.facid, MONTH(b.starttime) AS month, SUM(b.slots) 		as UsageAmount
      FROM  Bookings AS b
